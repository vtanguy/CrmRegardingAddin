...
namespace CrmRegardingAddin
{
    internal static class CrmActions
    {
        ...
        {
            try { Debug.WriteLine("[CRMADDIN] " + msg); } catch { }
        }

        // =================== MAIL: FROM/TO natif Outlook + parité MAPI ===================
        public static void SetRegarding(IOrganizationService org, EntityReference regarding, Outlook.MailItem mi)
        {
            ...
            // FROM
            var fromParties = new List<Entity>();
            if (isOutgoing && meId != Guid.Empty)
            {
                fromParties.Add(BuildActivityPartySystemUser(meId, 1));
            }
            else
            {
            ...
            // TO / CC / BCC
            var toParties  = BuildRecipients(org, toList,  2);
            var ccParties  = BuildRecipients(org, ccList,  3);
            var bccParties = BuildRecipients(org, bccList, 4);

            // Recherche par message-id
            var internetId = MailUtil.GetInternetMessageId(mi);
            Entity existing = null;
            try { existing = MailUtil.FindCrmEmailByMessageId(org, internetId); } catch { }
            ...
            if (existing == null)
            {
                ...
                email["description"]   = mi.HTMLBody ?? mi.Body ?? "";
                ...
                // ---- MS-compat Add-In: écrire les propriétés MAPI lues par l’addin Microsoft ----
                try
                {
                    // SMTP du user courant (systemuser)
                    var currentUserSmtp = GetCurrentUserSmtp(org);
                    var currentUserId = GetCurrentUserId(org); // Guid

                    // FROM SMTP réel (robuste : en-têtes, Sender, etc.)
                    var fromSmtp = GetSenderSmtpStrict(mi);

                    // Liste des destinataires SMTP (To/Cc/Bcc)
                    ...
                    // Unifié entrants/sortants (c’est MS qui s’en moque, il lit juste le partyinfo)
                    ...
                    // Affichage du "concernant" : si tu as le libellé, mets-le; sinon chaîne vide
                    var regardingDisplay = (regarding != null && !string.IsNullOrEmpty(regarding.Name))
                        ? regarding.Name
                        : "";

                    // isIncoming juste si tu en as déjà le calcul (sinon: bascule sur comparaison FROM==user)
                    bool isIncoming = true;
                    if (!string.IsNullOrEmpty(currentUserSmtp) && !string.IsNullOrEmpty(fromSmtp))
                        isIncoming = !currentUserSmtp.Equals(fromSmtp, StringComparison.OrdinalIgnoreCase);

                    // Appel interop MAPI (rendra l’élément "Suivi" visible dans l’addin Microsoft)
                    CrmMapiInterop.ApplyMsCompatForMail(
                        mi,
                        "",                              // regardingLogicalName/code si tu veux (sinon "")
                        (regarding != null) ? regarding.Id : Guid.Empty,
                        regardingDisplay,
                        currentUserSmtp,
                        (currentUserId != Guid.Empty) ? (Guid?)currentUserId : null,
                        fromSmtp,
                        allRecipients,
                        isIncoming
                    );
                }
                catch { /* ne bloque jamais l’enregistrement */ }
            ...
            // --- Parité addin Microsoft : pose les propriétés MAPI de tracking sur le MailItem ---
            Guid? regardingId = (regarding != null) ? (Guid?)regarding.Id : null;
            Guid? ownerId = (meId != Guid.Empty) ? (Guid?)meId : null;
            CrmMapiInterop.SetTracked(mi, activityId, regardingId, ownerId);
        }

        // =================== APPOINTMENT: organizer seul (pas de requiredattendees) + parité MAPI ===================
        public static void SetRegarding(IOrganizationService org, EntityReference regarding, Outlook.AppointmentItem appt)
        {
            ...
            if (meId != Guid.Empty)
            {
                ...
                organizer["participationtypemask"] = new OptionSetValue(7);
            ...
            if (existing == null)
            {
                ...
                Info("APPOINTMENT created in CRM: " + activityId);
            }
            else
            {
                org.Update(apptEntity);
                activityId = existing.Id;
                Info("APPOINTMENT updated in CRM: " + existing.Id);
            }
            // ---- MS-compat Add-In: écrire les propriétés MAPI lues par l’addin Microsoft ----
            try
            {
                var currentUserSmtp = GetCurrentUserSmtp(org);
                var currentUserId = GetCurrentUserId(org);
            ...
            // Parité addin Microsoft : marquer l’Appointment Outlook comme "suivi"
            Guid? regardingId = (regarding != null) ? (Guid?)regarding.Id : null;
            Guid? ownerId = (meId != Guid.Empty) ? (Guid?)meId : null;
            CrmMapiInterop.SetTracked(appt, activityId, regardingId, ownerId);
        }
        ...
        public static void UnlinkOrDeleteCrmEmail(IOrganizationService org, Outlook.MailItem mi, bool deleteInCrm)
        {
            ...
            var existing = MailUtil.FindCrmEmailByMessageId(org, msgId);
            if (existing != null && deleteInCrm)
            {
                try { org.Delete("email", existing.Id); } catch { }
            }

            // Parité Microsoft pour "conserver en CRM" : efface uniquement les propriétés MAPI de tracking
            CrmMapiInterop.ClearTracked(mi);
        ...
        public static void UnlinkOrDeleteCrmAppointment(IOrganizationService org, Outlook.AppointmentItem appt, bool deleteInCrm)
        {
            ...
            // Parité Microsoft : efface uniquement les flags MAPI du suivi
            CrmMapiInterop.ClearTracked(appt);

            try
            {
                var insp = appt.GetInspector;
                if (insp != null) Globals.ThisAddIn.CreatePaneForAppointmentIfLinked(insp, appt);
            } catch { }
        ...
        private static System.Collections.Generic.IEnumerable<string> ParseAddressList(Outlook.MailItem mi, string disp)
        {
