    ...
    public static class LinkingApi
    {
        ...
        public static void PrepareAppointmentLinkInOutlookStore(IOrganizationService org, Outlook.AppointmentItem appt, Guid regardingId, string regardingLogicalName, string regardingReadableName)
        {
            ...
            try {
                ...
                if (!string.IsNullOrWhiteSpace(__xml)) { TrySet(__pa, DaslStr.PartyInfo, __xml); TrySet(__pa, DaslId.PartyInfo, __xml); try { appt.Save(); } catch { } }
            } catch { }
        ...
        public static void FinalizeMailLinkInOutlookStoreAfterCrmCommit(IOrganizationService org, Outlook.MailItem mi, Guid createdEmailId)
        {
            ...
            // NEW: Build and set CrmPartyInfo for mail (To, Cc, Bcc, From)
            try
            {
                var xml = BuildPartyInfo(org, mi);
                if (!string.IsNullOrWhiteSpace(xml))
        ...
        // Legacy simple PartyInfo for mail (kept for compatibility but no longer used)
        private static string BuildPartyInfo(Outlook.MailItem mi)
        {
            try
            {
                ...
                AppendRecipientsLegacy(sb, mi, Outlook.OlMailRecipientType.olTo, "to");
                AppendRecipientsLegacy(sb, mi, Outlook.OlMailRecipientType.olCC, "cc");
                AppendRecipientsLegacy(sb, mi, Outlook.OlMailRecipientType.olBCC, "bcc");
                return sb.ToString();
            }
            catch { return null; }
        }
        ...
        private static string BuildPartyInfo(IOrganizationService org, Outlook.MailItem mi)
        {
            try
            {
                ...
                // Order to match MS add-in: To (2), Cc (3), Bcc (4), then From (1)
        ...
        private static void AppendMailFromWithCrm(IOrganizationService org, StringBuilder sb, Outlook.MailItem mi, string participation)
        {
            try
            {
                ...
                // Try to get richer name via AddressEntry if possible
                try
                {
                    var ae = mi.Sender;
                    if (ae != null)
        ...
        private static void AppendRecipientsLegacy(StringBuilder sb, Outlook.MailItem mi, Outlook.OlMailRecipientType type, string label)
        {
            try
            {
                bool first = true;
